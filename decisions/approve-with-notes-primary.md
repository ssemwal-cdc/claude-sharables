...
Approve With Notes is the only path that can attach the note. D‹approved-by-claude-comment›, default the approval comment, requires one.
...
Do not promote the direct approval URL to the primary route. See F‹plain-approve-noop-user-activation›, plain Approve can no-op.

**Evidence.**

- Recorded 2026-08-15 with the plain-Approve recovery.
- The freeze fallback is F‹approve-notes-same-tab-freeze›, the notes click is a navigation.
