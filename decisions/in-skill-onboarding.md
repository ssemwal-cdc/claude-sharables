...
2. **Setup advice, once.** On a first run, say in a few lines what works best. That is the latest Opus model and medium to high effort. It is also Cowork with Downloads connected. Name only the items the run can see are off. The model is readable. The mode is read from the tools present. Effort is not readable, so state it as advice.
