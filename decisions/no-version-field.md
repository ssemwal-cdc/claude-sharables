...
The rule bans the machine fields only. The human marker is required. See D‹skill-version-lines›, four synced version sites.

**Evidence.**

- Confirmed 2026-08-11. `claude plugin list` prints a commit SHA prefix as the version, for example `7ca6a2ea2c70`.
- `claude plugin validate` warns `No version specified`. That warning is expected here. See D‹plugin-validate-warning-expected›, expected validator warning.
