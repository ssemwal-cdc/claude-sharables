...
10. Mark eleven decisions `superseded`, each citing this record. They are D14, never pre-write an action, through D78, an unmapped subtype keeps buttons, and the list sits under Evidence. Close F‹execute-button-blocked-state-confirmed›, execute button blocked state, and F‹po-execute-route-confirmed›, purchase order route, as moot. Regenerate the indexes.
...
- No gap does now. Both closed 2026-09-24. See F‹execute-button-blocked-state-confirmed›, the
  amber state seen, and F‹po-execute-route-confirmed›, the maintainer's report that the route
  worked.
