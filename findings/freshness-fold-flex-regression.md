...
The first attempt moved the freshness line inside the header block. That block's left column is a flex item sized to its own content.

So a long line made that column wide, and wrapped the right-hand group onto a second row. That grew the header block instead of shrinking it.
...
- The landed version cut the chrome above the first item. It went from 454px to 380px on NetSuite and from 467px to 401px on Procore.
