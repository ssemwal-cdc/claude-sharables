...
Four rounds of probes followed. They included two wrong versions of a
maintainer note and an onboarding section written then deleted unshipped.
...
No tool returns an agent's own permission mode. So the single fact the
whole investigation turned on was unverifiable by the agent.
