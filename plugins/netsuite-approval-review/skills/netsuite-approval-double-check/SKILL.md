...
- **In browser mode, `get_page_text` on each record tab is the route.** Open **a record tab** for this read, or reuse the one Step 1 opened, and reuse it for Step 3 and Step 4 on the same item. Record tabs are the tabs this skill opens. A record tab closes once Step 5 has finished for that item. The warning above is a cost optimisation, not a prohibition. Read the field set off the page, including the Items sublist lines. Expect the run to be slower per item. That needs no comment.
