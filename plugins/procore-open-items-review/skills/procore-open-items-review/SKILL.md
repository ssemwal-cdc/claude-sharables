...
1. **Confirm Claude in Chrome is connected and Procore is authenticated.** Before navigating, warn the user about the site-access prompt: Claude in Chrome asks whether to allow access to a site the first time it acts on one, offering a once-only option and an always option. **Tell them to pick the always option** — once for Procore, and again for the S3 host when Step 4 first reads an attachment. On once-only they are re-prompted on effectively every action, and a run that stalls waiting for a prompt nobody is watching reads as a hang. Then navigate to the company Open Items tool.

   **Authentication has three outcomes and the run must say which one it is looking at.** Decide on what is on screen, never on what a rule sounds like it implies.

   | Outcome | On screen | What to do |
   |---|---|---|
   | `authenticated` | the Open Items tool loaded | touch no login form. Carry on to rung 2. |
   | `email-only` | an email field and a **Continue** button, and **no password box** | `find` the email field, set it with `form_input`, click **Continue**, stop there, then re-read the page and re-enter this table. **This step is permitted, not an exception** — it types no secret, and it is how a Compass session normally re-establishes itself through SSO. |
   | `wall` | a password box, an MFA prompt, or a CAPTCHA — whether or not an email field sits beside it | hand off, as specified below. Type into none of them and retry nothing. |

   **A hand-off is an action, not the end of the run.** Say in chat that Procore is showing a sign-in screen, name the field that made it a `wall`, and ask the user to sign in to Procore in that Chrome profile and say when they have. Then re-read the page and carry on with the review in the same run. Where nobody is watching — a scheduled window — the ask is still what goes in the report, because it is the only thing that can fix this, and the run ends there.

   **Leave `lastCompletedRun` unchanged, since nothing was reviewed — but never report that a later window will retry it.** An expired session does not heal on its own, so every window until someone signs in fails identically. Saying a retry is coming tells the user their queue is being handled when it is not, which is worse than saying nothing.

   **The teammate-facing sheet's promise is that this skill never types your password. It is not a prohibition on the `email-only` step.** Observed 2026-09-10: a run reached a sign-in screen, cited a "hard constraint" that appears nowhere in this file, reviewed nothing, and reported that the next scheduled window would retry. What it was reading was an earlier version of this rung, which stated that promise as a bolded absolute and demoted the email step to a subordinate clause underneath it — so a refusal was assembled out of this skill's own words and sounded authoritative doing it. There is no rule here against the `email-only` step, and a run that declines it is misreading rather than being careful. If the screen genuinely carried a password box, `wall` was the right call and only the wording and the recovery were wrong. NetSuite's skill has no login procedure at all and does not need one; this is the single asymmetry between the two plugins on authentication.
