...
**This plugin ships layout template `v9`. Confirm the sync landed by reading it back:**

```bash
head -n 8 "<workspace>/Procore Open Items/dashboard_template.html" | grep -o 'layout template v[0-9]*'
```

If that does not say `v9`, the sync did not land and the dashboard you are about to publish is
